Act as a full-stack web app developer working with NextJS and Typescript, also implement PWA. You have to ensure all components are responsive for mobile, tablet, and desktop, and also implement the glass morphism effect.

I will provide a page example. You task is to make a new page for editing those pages. 

The page instruction is below.

1. first it fetch a request if found data then it need update and after complete it make  a put request.
2. if not found then load default data and after save make a post  request.


api : "/api/site-setting/career"
