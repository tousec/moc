...
type ScorePlan = { id: string; targetScore: string; title: string; description: string; };
type ScholarshipPathStep = { id: string; step: number; icon: string; title: string; description: string; };
type CareerPathPageData = { pageTitle: string; pageSubtitle: string; statistics: Statistic[]; scorePlans: ScorePlan[]; scholarshipPathSteps: ScholarshipPathStep[]; requiredDocuments: string[]; };

// ---------------------------
// Default Data
// ---------------------------

const DEFAULT_DATA: CareerPathPageData = {
  pageTitle: "Your Career Path to Global Opportunities",
  pageSubtitle: "Leverage your IELTS score to unlock funded scholarships and study at top universities abroad.",
  statistics: [{ id: "s1", icon: "🎓", value: "1,000+", label: "Students Abroad" }],
  scorePlans: [{ id: "p1", targetScore: "Band 7+", title: "The Gateway Score", description: "Achieving a 7.0-7.5+ band is the crucial first step." }],
  scholarshipPathSteps: [{ id: "step1", step: 1, icon: "🎯", title: "Achieve a High Score", description: "Secure a competitive IELTS (7.5+) and GRE/GMAT score." }],
  requiredDocuments: ["Passport & National ID", "Academic Transcripts & Certificates"],
};
...
const StringListEditor: FC<{ list: string[]; setList: (list: string[]) => void; label: string }> = ({ list, setList, label }) => {
    ...
    return ( <div className="space-y-2"><label className="block text-sm font-medium text-white/90 mb-1">{label}</label>{list.map((item, index) => ( <div key={index} className="flex items-center gap-2"><p className="flex-grow p-2 bg-black/20 rounded-md text-sm text-white/80">{item}</p><button type="button" onClick={() => handleRemoveItem(index)} className="text-red-400 hover:text-red-300 text-xl">&times;</button></div> ))}<div className="flex gap-2"><GlassInput value={newItem} onChange={(e) => setNewItem(e.target.value)} placeholder={`Add new...`} className="py-2" /><button type="button" onClick={handleAddItem} className="px-4 py-2 text-sm rounded-lg font-semibold bg-teal-500/80 text-white hover:bg-teal-500/100">Add</button></div></div> );
};
...
export default function EditCareerPathPage() {
  const [pageData, setPageData] = useState<CareerPathPageData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/api/site-setting/career-path");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/site-setting/career-path", { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(pageData), });
  ...
  const handleFieldUpdate = <K extends keyof CareerPathPageData>(field: K, value: CareerPathPageData[K]) => { setPageData(prev => prev ? { ...prev, [field]: value } : null); };
  const handleObjectListItemUpdate = <K extends ObjectArrayKeysOf<CareerPathPageData>>(listKey: K, index: number, field: keyof CareerPathPageData[K][number], value: any) => { setPageData(prev => { if (!prev) return null; const list = prev[listKey]; const updatedList = list.map((item, i) => i === index ? { ...item, [field]: value } : item); return { ...prev, [listKey]: updatedList }; }); };
  const handleListRemove = <K extends ObjectArrayKeysOf<CareerPathPageData>>(listKey: K, index: number) => { setPageData(prev => { if (!prev) return null; const list = prev[listKey]; const updatedList = list.filter((_, i) => i !== index); return { ...prev, [listKey]: updatedList }; }); };
  const handleListAdd = <K extends ObjectArrayKeysOf<CareerPathPageData>>(listKey: K, newItem: CareerPathPageData[K][number]) => { setPageData(prev => { if (!prev) return null; const list = prev[listKey]; const updatedList = [...list, newItem]; return { ...prev, [listKey]: updatedList }; }); };
  ...
  return (
    <main className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 p-4 md:p-8 lg:p-12"><div className="max-w-7xl mx-auto space-y-6">
          <GlassCard><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4"><div><h1 className="text-3xl font-bold text-white">Career Path Page Editor</h1><p className="text-white/80 mt-2">Manage content for the study abroad & scholarship page.</p></div><PrimaryButton onClick={handleSave} disabled={isSaving}>{isSaving ? 'Saving...' : 'Save Changes'}</PrimaryButton></div></GlassCard>
          ...
          <GlassCard><h2 className="text-2xl font-bold text-white mb-6">Step 1: Target Score Plans</h2>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {pageData.scorePlans.map((plan, index) => (
                <div key={plan.id} className="bg-white/5 p-4 rounded-lg space-y-3"><GlassInput value={plan.targetScore} onChange={e => handleObjectListItemUpdate('scorePlans', index, 'targetScore', e.target.value)} /><GlassInput placeholder="Title" value={plan.title} onChange={e => handleObjectListItemUpdate('scorePlans', index, 'title', e.target.value)} /><GlassTextarea placeholder="Description" value={plan.description} onChange={e => handleObjectListItemUpdate('scorePlans', index, 'description', e.target.value)} /></div>
              ))}
            </div>
          </GlassCard>

          <GlassCard>
            <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold text-white">Path to Scholarship</h2><button onClick={() => handleListAdd('scholarshipPathSteps', {id: `s${Date.now()}`, step: pageData.scholarshipPathSteps.length+1, title: '', description: '', icon: ''})} className="px-4 py-2 text-sm rounded-xl bg-purple-500/80 hover:bg-purple-500/100">+ Add Step</button></div>
            <div className="space-y-4">
              {pageData.scholarshipPathSteps.map((step, index) => (
                <div key={step.id} className="bg-white/5 p-3 rounded-lg grid grid-cols-[auto_1fr_auto] gap-3 items-center">
                  <div className="flex items-center gap-3"><span className="w-8 h-8 flex items-center justify-center bg-black/20 rounded-full shrink-0">{index+1}</span><GlassInput value={step.icon} onChange={e => handleObjectListItemUpdate('scholarshipPathSteps', index, 'icon', e.target.value)} className="w-16 text-center" /></div>
                  <div className="space-y-2"><GlassInput placeholder="Title" value={step.title} onChange={e => handleObjectListItemUpdate('scholarshipPathSteps', index, 'title', e.target.value)} /><GlassTextarea rows={2} placeholder="Description" value={step.description} onChange={e => handleObjectListItemUpdate('scholarshipPathSteps', index, 'description', e.target.value)} /></div>
                  <DangerButton onClick={() => handleListRemove('scholarshipPathSteps', index)}>Remove</DangerButton>
                </div>
              ))}
            </div>
          </GlassCard>

          <GlassCard><h2 className="text-2xl font-bold text-white mb-6">Required Documents</h2><StringListEditor list={pageData.requiredDocuments} setList={(list) => handleFieldUpdate('requiredDocuments', list)} label="Documents List"/></GlassCard>
